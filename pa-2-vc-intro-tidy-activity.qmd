...
**NAMES:** Riley Calbow and Luis Cerrillos
...
```{r}
#| label: create-vectors-letters-and-punctuation
...
```{r}
#| label: create-one-long-vector

my_symbols <- tibble(symbols = c(lower_case, 
                                 upper_case, 
                                 punctuation)) |> #combines into vector and names it my_symbols
              mutate(num = row_number()) #creates dataframe where each symbol is assigned to a number
```
...
```{r}
#| label: Altered the top secret set of numbers, prioritized using tibble functions
#opted for the method that would allow for us to not overwrite and have access to the previous numbers in case of error
...
```{r}
#| label: add-label-3
#Checking how many numbers are below 17

code_break |>
  summarize(total = sum(step9 <  17)) #using summarize function to get our total amount of numbers less than 17
...
```{r}
#| label: turn vector into a matrix with 5 columns
# turns vector into matrix, specifies 5 columns and names them V1-5 and id
...
```{r}
#| label: create vector with even numbers from 1 - 382
# creates vector with evens of interest and names it evens
